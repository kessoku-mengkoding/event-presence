...
@section('content')
  <div class="relative mx-auto my-12 max-w-2xl">
    <h1 class="my-6 text-2xl font-bold"><span class="text-gradient-purple-coral">{{ $event->name }}</span> <span
        class="text-gray-300">/</span> Edit Jadwal</h1>
    <form action="{{ route('updateTimetable') }}" method="POST" class="flex flex-col">
      @csrf
      @method('PUT')
      <input hidden type="text" name="id" value="{{ $timetable->id }}" class="input">
      <input hidden type="text" name="event_id" value="{{ $event->id }}" class="input">
      <label for="" class="font-semibold">Title</label>
      <input type="text" name="title" id="" value="{{ old('title') ?? $timetable->title }}"
        class="input mt-2"> @error('title')
      ...
      <div class="mt-6 flex w-full gap-6">
        <div class="flex w-full flex-col">
          ...
          <input type="datetime-local" name="start" id="startDatetime" value="{{ old('start') ?? $timetable->start }}"
            class="input mt-2">
          @error('start')
            <div class="mt-0.5 text-sm text-red-500">{{ $message }}</div>
          @enderror
        ...
        <div class="flex w-full flex-col">
          ...
          <input type="datetime-local" name="end" id="endDatetime" value="{{ old('end') ?? $timetable->end }}"
            class="input mt-2">
          @error('end')
            <div class="mt-0.5 text-sm text-red-500">{{ $message }}</div>
          @enderror
      ...
      <input type="number" name="lateness_tolerance"
        value="{{ old('lateness_tolerance') ?? $timetable->lateness_tolerance }}" id="" class="input mt-2">
      ...
      <div class="mt-6 flex w-full gap-6">
        <div class="flex w-full flex-col">
          ...
          <input type="text" name="lat" id="lat" value="{{ old('lat') ?? $timetable->latitude }}"
            class="input mt-2">
          @error('lat')
            <div class="mt-0.5 text-sm text-red-500">{{ $message }}</div>
          @enderror
        ...
        <div class="flex w-full flex-col">
          ...
          <input type="text" name="long" id="long" value="{{ old('long') ?? $timetable->longitude }}"
            class="input mt-2">
          @error('long')
            <div class="mt-0.5 text-sm text-red-500">{{ $message }}</div>
          @enderror
      ...
      <input type="number" name="radius_meter" id="radius"
        value="{{ old('radius_meter') ?? $timetable->radius_meter }}" value="100" class="input mt-2">
      @error('radius_meter')
        <div class="mt-0.5 text-sm text-red-500">{{ $message }}</div>
      @enderror
  ...
  <script>
    ...
    const lat = {{ $timetable->latitude }}
    const long = {{ $timetable->longitude }}
